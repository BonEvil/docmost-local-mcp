...
**Supported build note.** Release builds for macOS enable the `keyring` crate's
Apple-native backend and verify a write/read/delete round trip against Keychain. The
supported headless Linux Atlas host has no desktop secret service, so the keyring path
still fails closed on that target. Session-only authentication remains the recommended
headless-Linux mechanism; unattended reauthentication there requires the operator to
accept the weaker encrypted-file fallback explicitly. Do not read a successful
`remember_password` request as proof that an OS keyring is in use on another platform.
